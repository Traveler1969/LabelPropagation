...
public class InverseListBuilder {
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(InverseListBuilder.class);
        // 从命令行参数中获取输入路径、输出路径
        Path inputPath = new Path(args[0]);
        Path outputPath = new Path(args[1]);
        // 设置作业的Mapper相关信息
        job.setMapperClass(ListBuildMapper.class);
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(Text.class);
        // 设置作业的Reducer相关信息
        job.setReducerClass(ListBuildReducer.class);
